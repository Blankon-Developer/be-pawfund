...
func (r *PostgresFundraiserRepository) ReplaceProfile(
	...
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return repository.ReplaceFundraiserProfileResult{}, false, fmt.Errorf("repository: begin replace fundraiser profile transaction: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	...
	if err := tx.QueryRowContext(ctx, lockFundraiser, strings.TrimSpace(walletAddress)).Scan(
		...
		return repository.ReplaceFundraiserProfileResult{}, false, fmt.Errorf("repository: lock fundraiser profile for replacement: %w", err)
	}
	...
	if _, err := tx.ExecContext(ctx, replaceUserEmail, profile.Email, fundraiser.ID); err != nil {
		return repository.ReplaceFundraiserProfileResult{}, false, mapPostgresError("replace fundraiser user email", err)
	}
	...
	if _, err := tx.ExecContext(
		...
		return repository.ReplaceFundraiserProfileResult{}, false, mapPostgresError("replace fundraiser profile", err)
	}
	...
	if profile.ImageObjectKey.Set {
		...
		if _, err := tx.ExecContext(ctx, replaceImageObjectKey, profile.ImageObjectKey.Value, fundraiser.ID); err != nil {
			return repository.ReplaceFundraiserProfileResult{}, false, mapPostgresError("replace fundraiser image object key", err)
		}
	}

	result := repository.ReplaceFundraiserProfileResult{}
	if profile.ImageObjectKey.Set && currentImageObject.Valid && imageObjectKeyChanged(currentImageObject.String, profile.ImageObjectKey.Value) {
		...
		if err := tx.QueryRowContext(ctx, imageStillReferenced, currentImageObject.String).Scan(&referenced); err != nil {
			return repository.ReplaceFundraiserProfileResult{}, false, fmt.Errorf("repository: check old fundraiser image references: %w", err)
		}
		if !referenced {
	...
	if err := tx.Commit(); err != nil {
		return repository.ReplaceFundraiserProfileResult{}, false, fmt.Errorf("repository: commit replace fundraiser profile transaction: %w", err)
	}
...
func (r *PostgresFundraiserRepository) DeleteProfile(
	...
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
		return repository.DeleteFundraiserProfileResult{}, false, fmt.Errorf("repository: begin delete fundraiser profile transaction: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	...
	if err := tx.QueryRowContext(ctx, lockFundraiser, strings.TrimSpace(walletAddress)).Scan(
		...
		return repository.DeleteFundraiserProfileResult{}, false, fmt.Errorf("repository: lock fundraiser profile for deletion: %w", err)
	}
	...
	if err := tx.QueryRowContext(ctx, hasActiveCampaign, fundraiserID).Scan(&activeCampaignExists); err != nil {
		return repository.DeleteFundraiserProfileResult{}, false, fmt.Errorf("repository: check fundraiser active campaigns: %w", err)
	}
	if activeCampaignExists {
		return repository.DeleteFundraiserProfileResult{}, true, repository.ErrFundraiserHasActiveCampaigns
	}
	...
	if _, err := tx.ExecContext(ctx, clearFundraiserImage, fundraiserID); err != nil {
		return repository.DeleteFundraiserProfileResult{}, false, fmt.Errorf("repository: clear fundraiser profile image: %w", err)
	}
	...
	if _, err := tx.ExecContext(ctx, softDeleteUser, fundraiserID); err != nil {
		return repository.DeleteFundraiserProfileResult{}, false, fmt.Errorf("repository: soft delete fundraiser profile: %w", err)
	}

	result := repository.DeleteFundraiserProfileResult{}
	if currentImageObject.Valid {
		...
		var referenced bool
		if err := tx.QueryRowContext(ctx, imageStillReferenced, currentImageObject.String).Scan(&referenced); err != nil {
			return repository.DeleteFundraiserProfileResult{}, false, fmt.Errorf("repository: check deleted fundraiser image references: %w", err)
		}
		if !referenced {
	...
	if err := tx.Commit(); err != nil {
		return repository.DeleteFundraiserProfileResult{}, false, fmt.Errorf("repository: commit delete fundraiser profile transaction: %w", err)
	}
